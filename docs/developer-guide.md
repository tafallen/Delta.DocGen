...
**Implementation status:** Stories 1–18 complete (177 tests passing)
...
Options:
  ...
  --no-exclude-config  Suppress excludes loaded from the config file (CLI --exclude still applies)
```

CLI arguments always take precedence over config file values. `--exclude` is additive (does not replace config excludes) unless `--no-exclude-config` is also passed.
...
| Stories | Status |
|---------|--------|
| 1–18 | ✅ Complete, merged to master, pushed to GitHub |
